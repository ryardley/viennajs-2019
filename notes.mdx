...
#### Predicates in JS are not concise enough
...
<Notes>
  <li>PDSL converts a p-expression to a predicate function</li>
</Notes>
...
- Syntax highlighting and autosuggest
- Not all Syntax is defined (Issues and PRs accepted)
- Moving to exact matching soon
- Validation errors soon for use in form libraries
...
<Code
  size={25}
>{`p\`[ *, *, string, ... ]\`(\n  [1,null,'two', 3]\n); // true\n\np\`{ age: number, name: string, ... }\`(\n  {name:'foo', age: 12, color: 'black'}\n); // true\n\np\`{ age: number, name: string }\`(\n  {name:'foo', age: 12, color: 'black'}\n); // false`}</Code>

---

### Soon: Validation errors

<Code size={25}>{`const isVerified = p\`{
  ...
  name: 'Fred',
  age: 21
}); // ["Length must be greater than 7"]`}</Code>
